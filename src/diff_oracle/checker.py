...
class Checker:
    ...
    def C(self, x: str):
        ...
        if c_error or self.c_handler.get_exit_code() != 0:
            return res.construct_error(c_error, self.c_handler.get_exit_code())
    ...
    def R(self, x: str):
        ...
        if r_error or self.r_handler.get_exit_code() != 0:
            return res.construct_error(r_error, self.c_handler.get_exit_code())
    ...
    def batch_test(self, n: int, test_cases: list, exec_func):
        with Pool(processes=6) as pool:
            ...
            # results = pool.map(self.one_d_int_step_basin_hopping, args_list)
