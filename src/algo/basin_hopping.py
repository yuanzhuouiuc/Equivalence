...
class Basin_Hopping:
    ...
    def one_d_int_step_basin_hopping(args):
        ...
        bounds = [(constant.Constant.INT_LOWER_BOUND, constant.Constant.INT_UPPER_BOUND) for _ in range(n)]
        integer_step = int_step.IntegerStep(stepsize=50)

        minimizer_kwargs = {"method": "L-BFGS-B", "bounds": bounds}
        result = basinhopping(checker.int_step_objective, x0, minimizer_kwargs=minimizer_kwargs, niter=10000,
    ...
    def one_d_unicode_basin_hopping(args):
        ...
        integer_step = int_step.IntegerStep(stepsize=3)

        minimizer_kwargs = {"method": "L-BFGS-B", "bounds": bounds}
        result = basinhopping(checker.unicode_objective, x0, minimizer_kwargs=minimizer_kwargs, niter=20000,
                              take_step=integer_step, T=1.0)
        max_difference = -result.fun
        print(f"Test case {idx + 1}: Max |C(x) - R(x)| = {max_difference}")
        return max_difference, result
